...
/**
 * Mirrors Prisma's OpportunityStatus. Kept as a literal union rather than
 * `string` so a predicate testing a status the database cannot produce fails
 * to compile — that is exactly how the 'WON' bug survived: `string` accepted
 * it, and the unit tests fed the same impossible value.
 */
...
// A won deal carries status CLOSED — there is no 'WON' member in
// OpportunityStatus (ACTIVE | ON_HOLD | CANCELLED | LOST | CLOSED), and
// changing status to CLOSED is what sets forecastCategory to CLOSED_WON.
// This predicate previously tested for 'WON', which nothing can ever produce,
// so WIN_RATE, AVERAGE_DEAL_SIZE and FORECAST_ACCURACY were pinned at zero.
